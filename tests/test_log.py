...
def test_generate_text_log(caplog):
    ...
    for record in caplog.records:
        assert (
            record.msg
            == u"Key account treat vote old public base short door air small get perhaps past chance consumer boy address main who. "
        )
...
def test_generate_text_log_with_more_words(caplog):
    ...
    for record in caplog.records:
        assert (
            record.msg
            == u"Key account treat vote old public base short door air small get perhaps past chance consumer boy address main who ask must letter own become often especially. "
        )
